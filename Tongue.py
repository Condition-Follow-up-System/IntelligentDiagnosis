...
# main
imagesrc = cv2.imread("C:/test1.jpg")
gray = cv2.cvtColor(imagesrc, cv2.COLOR_BGR2GRAY)
src1= cv2.cvtColor(imagesrc, cv2.COLOR_BGR2RGB) 
m = 3  #阈值数
...
plt.subplot(131), plt.imshow(src1)
...
src=cv2.imread("C:/s2.webp")
gray=cv2.cvtColor(src,cv2.COLOR_BGR2GRAY)
src1= cv2.cvtColor(src, cv2.COLOR_BGR2RGB)
plt.subplot(131),plt.imshow(src1)
plt.title("source image"),plt.xticks([]),plt.yticks([])
plt.subplot(132),plt.hist(src.ravel(),256)
plt.title("Histogram"),plt.xticks([]),plt.yticks([])
ret1,th1=cv2.threshold(gray,0,255,cv2.THRESH_OTSU)
plt.subplot(133),plt.imshow(th1,"gray")
plt.title("OSTU,threshold is "+str(ret1)),plt.xticks([]),plt.yticks([])
plt.show()



'''import cv2
a=cv2.imread("c:/s1.webp")
